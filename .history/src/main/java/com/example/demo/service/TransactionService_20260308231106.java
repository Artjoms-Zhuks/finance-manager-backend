...
public class TransactionService {
    ...
    public Transaction createTransaction(Transaction transaction) {
        ...
        // 2. Проверяем сумму (используем compareTo, так как это BigDecimal)
        ...
        if (transaction.getAmount() == null || transaction.getAmount().compareTo(java.math.BigDecimal.ZERO) <= 0) {
            System.out.println("(!) Warning! Added a strange amount: " + transaction.getAmount());
        }
    ...
    public Transaction updateTransaction(Long id, Transaction details) {
        return repository.findById(id)
                .map(transaction -> {
                    ...
                    transaction.setCategory(details.getCategory().toUpperCase());
                    return repository.save(transaction);
                })
